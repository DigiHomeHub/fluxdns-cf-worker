...
export function loadConfig(env) {
  ...
  try {
    ...
    console.log('No valid configuration source found, using default configuration');
    return DEFAULT_CONFIG;
  } catch (error) {
...
function createBasicConfig(env) {
  const upstream = env.UPSTREAM_DNS || 'https://security.cloudflare-dns.com/dns-query';
...
function createAdBlockConfig(env) {
  const upstream = env.UPSTREAM_DNS || 'https://security.cloudflare-dns.com/dns-query';
  const timeout = parseInt(env.TIMEOUT || '5000', 10);
  const blockListUrl = env.BLOCKLIST_URL || 'https://raw.githubusercontent.com/StevenBlack/hosts/master/hosts';
...
function createSplitConfig(env) {
  const defaultUpstream = env.DEFAULT_UPSTREAM || 'https://security.cloudflare-dns.com/dns-query';
  const alternateUpstream = env.ALTERNATE_UPSTREAM || 'https://dns.google/dns-query';
  const timeout = parseInt(env.TIMEOUT || '5000', 10);
  
  // Domains to route to alternate upstream (comma-separated list)
  const alternateDomains = (env.ALTERNATE_DOMAINS || '').split(',')
    .map(d => d.trim())
    .filter(Boolean);
