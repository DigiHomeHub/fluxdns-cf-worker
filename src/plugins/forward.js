...
export async function executeForward(ctx, args) {
  ...
  try {
    ...
    const fetchOptions = {
      ...
      body: ctx.dnsMessage.buffer,
      cf: { cacheTtl: 300 } // Use Cloudflare edge cache when possible
    };
    
    // Set up request timeout
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort('DNS query timeout'), timeout);
    fetchOptions.signal = controller.signal;
    
    // Add EDNS Client Subnet if requested
    if (edns_client_subnet && ctx.metadata.clientInfo && ctx.metadata.clientInfo.ip) {
    ...
    if (!response.ok) {
      throw new Error(`Upstream DNS server returned ${response.status}: ${response.statusText}`);
    }
    ...
    ctx.metadata.stats.upstreamResponseTime = Date.now() - ctx.metadata.timings.start;
